...
def get_masking_bounds(df, window_length_mins=120):
    """
    The start and end of the light pulse window.
    
    Parameters:
    df: pandas dataframe -- the dataframe to find the window for
    window_length_mins: the approximate length of the light pulse; doesn't need to be exact
...
def calculate_spider_activity(df, start_time, duration_mins=60):
    ...
    start_time: datetime -- start of measurement period
    duration_mins: int -- length of measurement period
...
def get_masking_data(pulse_file, control_series, duration_mins=60):
    ...
    start of the pulse, control group mean activity during pulse, individual pulse group spider activity during pulse

    Parameters:
    pulse_file: str -- file name to measure from
    control_series: pandas Series -- activities of control group across one ZT day
    duration_mins: int -- duration of measurement
    """
