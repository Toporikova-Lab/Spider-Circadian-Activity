...
class Perceptron():
	...
	def squash(self, net): # I HAVE NO CLUE WHAT THIS FUNCTION DOES OR WHY IT WORKS
	...
	def train(self, I, T, neps=1000, eta=0.05, mew=0):
		...
		for i in range(neps): # just a nice-looking thing
			...
			DwIHprev = DwIH
			DwHOprev = DwHO
			loss_counter.append(np.abs(runningLoss/p))

		print(progressBar(message='Done!'))
		plt.plot([i for i in range(1,neps+1)], loss_counter)
		plt.xlabel("Epoch")
		plt.ylabel("Training Loss")
		plt.title("Shallow MLP : episodes={}, momentum={}, weight decay={}".format(neps, mew, eta))
		plt.savefig(os.path.join('modules', 'saves', 'shallowe.png'))
